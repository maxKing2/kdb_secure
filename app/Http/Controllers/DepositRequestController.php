...
class DepositRequestController extends Controller {
    ...
    public function get_table_data(Request $request) {
        ...
        return Datatables::eloquent($deposit_requests)
            ...
            ->addColumn('action', function ($deposit_request) {
                ...
                $actions .= '<a href="' . action('DepositRequestController@show', $deposit_request['id']) . '" data-title="' . _lang('Deposit Request') . '" class="btn btn-outline-primary btn-sm ajax-modal"><i class="icofont-eye-alt"></i> ' . _lang('Details') . '</a>&nbsp;';
    ...
    public function reject($id) {
        ...
        try {
            $depositRequest->user->notify(new RejectDepositRequest($depositRequest));
        } catch (\Exception$e) {}
